...
<!--  Basic MODS  -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:foxml="info:fedora/fedora-system:def/foxml#" xmlns:mods="http://www.loc.gov/mods/v3" xmlns:etdms="http://www.ndltd.org/standards/metadata/etdms/1.0/" version="1.0" exclude-result-prefixes="mods etdms">
  <!--
...
<!--  
  ...
  <xsl:template match="mods:*[(@type='date') or (contains(translate(local-name(), 'D', 'd'), 'date'))][normalize-space(text())]" mode="slurping_MODS">
    ...
    <xsl:variable name="this_prefix">
        <xsl:value-of select="$prefix"/>
        <xsl:for-each select="not(@encoding)">
          <xsl:value-of select="local-name()"/>
          <xsl:text>_</xsl:text>
          <xsl:value-of select="."/>
          <xsl:text>_</xsl:text>
      ...
      <field>
        <xsl:attribute name="name">
          <xsl:value-of select="concat($this_prefix, local-name(), '_mdt')"/>
        </xsl:attribute>
        <xsl:value-of select="$textValueISO"/>
      </field>
      <xsl:if test="concat($this_prefix, local-name())='mods_originInfo_dateIssued' or concat($this_prefix, local-name())='mods_originInfo_dateCreated'">
          ...
          <field>
          <xsl:attribute name="name">
              <xsl:value-of select="concat($this_prefix, 'mods_originInfo_dateIssued_and_dateCreated_mdt')"/>
          </xsl:attribute>
          <xsl:value-of select="$textValueISO"/>
          </field>
      </xsl:if>
  ...
  <xsl:template match="*" mode="slurping_MODS">
    ...
    <xsl:template name="collection">
        ...
        <xsl:for-each select="mods:extension/localCollectionName">
            ...
            <xsl:if test=".='c21' or .='crr' or .='cwp' or .='scaw' or .='timss-pirls' or .='wfrn'">
                <xsl:variable name="this_prefix">         
                    <xsl:value-of select="concat($prefix, 'local_collection')"/>       
                </xsl:variable>
                <xsl:variable name="textValue">   
        ...
        <xsl:for-each select="mods:extension/etdms:degree/etdms:discipline">
            ...
            <xsl:variable name="textValue">
                <xsl:choose>
                    ...
                    <xsl:when test="contains(.,'Organization Studies and Corporate Responsibility')">
